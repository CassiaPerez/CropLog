...
const SettingsView: React.FC<SettingsViewProps> = (props) => {
  const { apiConfig, setApiConfig, isSyncing, syncError, handleSaveSettings, handleSyncErp } = props;
  // ... (no changes needed)
  return (
      <div className="space-y-8 animate-in fade-in duration-500">
...
const LoadMapsPlannerView: React.FC<LoadMapsPlannerViewProps> = (props) => {
  const { loadMaps, setSelectedMapId, setCurrentView, getStatusColor, getLoadProgress, formatCurrency, getLogoAsBase64 } = props;
...
const SeparationListView: React.FC<SeparationListViewProps> = (props) => {
  const { loadMaps, setSelectedMapId, setCurrentView, getStatusColor } = props;
  // ... (no changes needed)
...
const PlanningMapDetailView: React.FC<PlanningMapDetailViewProps> = (props) => {
  const { loadMaps, selectedMapId, setLoadMaps, setCurrentView, addTimelineEvent, formatCurrency, getStatusColor, getEmbedUrl, saveLoadMapToDatabase, deleteLoadMapFromDatabase, updateInvoiceAssignedStatus, setInvoices, getLogoAsBase64 } = props;
...
const OperationListView: React.FC<OperationListViewProps> = (props) => {
  const { loadMaps, setSelectedMapId, setCurrentView, getStatusColor } = props;
  // ... (no changes needed)
...
const OperationDetailView: React.FC<OperationDetailViewProps> = (props) => {
  const { loadMaps, selectedMapId, setLoadMaps, setCurrentView, addTimelineEvent, formatCurrency, getStatusColor, getEmbedUrl } = props;
  // ... (no changes needed)
  const map = loadMaps.find(m => m.id === selectedMapId);
  if (!map) return <div>Mapa não encontrado</div>;
...
function App() {
  ...
  if (currentView === 'LOGIN') {
      return <LoginView users={users} setCurrentUser={setCurrentUser} setCurrentView={setCurrentView} />;
  }
